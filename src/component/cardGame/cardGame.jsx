...
class CardGame extends Component {
  ...
  onCardClick = (e) => {
    ...
    const errorMessage = `   player${arrayClicked +
      1} has already thown his card`;
    ...
    return (
      <div id="card-holder">
        {this.state.arraysOfPlayer.map((array, key) => {
          return (
            <div key={key} id={'player' + (1 + key)}>
              {array.map((value, keys) => {
                return (
                  <React.Fragment key={keys}>
                    <img
                      src={require(`./image/${value}`)}
                      name={value}
                      id={key}
